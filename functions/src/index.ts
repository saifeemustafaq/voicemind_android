...
export const findUserByEmail = onCall(async (request) => {
  ...
  try {
    ...
    const discoverable = profileDoc.exists
      ? profileDoc.data()?.discoverable ?? true
      : true;
...
export const shareItem = onCall(async (request) => {
  ...
  if (!itemId || !itemType || !recipientUid) {
    throw new HttpsError(
      "invalid-argument",
      "itemId, itemType, and recipientUid are required"
    );
  }
  ...
  // Verify item exists and caller owns it
  ...
  // Read profiles for denormalized names
  ...
  // Add recipientUid to item's sharedWith array
  batch.update(itemRef, {
    sharedWith: admin.firestore.FieldValue.arrayUnion(recipientUid),
  });
  ...
  // Create outbox entry for caller
  ...
  // For recordings, also update sharedWith on associated action items
...
export const revokeShare = onCall(async (request) => {
  ...
  if (!shareId || !recipientUid) {
    throw new HttpsError(
      "invalid-argument",
      "shareId and recipientUid are required"
    );
  }
...
export const getSharedAudioUrl = onCall(async (request) => {
  ...
  if (!ownerUid || !recordingId) {
    throw new HttpsError(
      "invalid-argument",
      "ownerUid and recordingId are required"
    );
  }

  const recordingRef = db.doc(`users/${ownerUid}/recordings/${recordingId}`);
  const recordingDoc = await recordingRef.get();
  if (!recordingDoc.exists) {
    throw new HttpsError("not-found", "Recording not found");
  }
